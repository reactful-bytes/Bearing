...
| M6.6      | not-started | Remove obsolete provider integration and migrate legacy event data       | Remove provider UI/services/gates and ICS import; normalize imported records as Bearing-owned events     |
| M6.6a     | not-started | Remove provider connection code and tests                                | Delete Google/Microsoft connection types, hooks, services, environment checks, and Profile controls      |
| M6.6b     | not-started | Remove ICS import and mirrored-event creation                            | Keep general ICS export while deleting parser, picker, and mirror writes                                 |
| M6.6c     | not-started | Migrate legacy calendar records                                          | Preserve existing imported events, retire provider-only fields, and document optional connection cleanup |
| M6.6d     | not-started | Remove calendar premium gates and copy                                   | Device calendar access is free core functionality                                                        |
| M6.7      | not-started | Add native calendar module, permissions, and adapter                     | Expo SDK 57 object API works in iOS/Android development builds                                           |
| M6.7a     | not-started | Install and configure native dependencies                                | Add `expo-calendar`, AsyncStorage, iOS full-access copy, and Android read/write permissions              |
| M6.7b     | not-started | Implement SDK 57 object API adapter                                      | Do not use deprecated `*Async` calendar APIs or `expo-calendar/legacy`                                   |
| M6.7c     | not-started | Add platform guard and injectable test double                            | Web, Jest, and unsupported environments fail gracefully                                                  |
| M6.7d     | not-started | Implement calendar permission lifecycle                                  | Handle unavailable, undetermined, granted, denied, blocked, and settings recovery states                 |
| M6.7e     | not-started | Establish development-build workflow                                     | Calendar testing does not depend on Expo Go                                                              |
| M6.8      | not-started | Add per-device calendar settings                                         | Each signed-in account selects visible calendars and one writable destination per device                 |
| M6.8a     | not-started | Discover and normalize system calendars                                  | Preserve title, color, source, visibility, primary status, access, and modification capability           |
| M6.8b     | not-started | Persist per-account device preferences                                   | Namespace selected IDs, default ID, and link cache by Firebase UID in AsyncStorage                       |
| M6.8c     | not-started | Build Profile calendar settings UI                                       | Show permission, visible-calendar choices, writable default, refresh, and settings recovery              |
| M6.8d     | not-started | Recover from stale calendar choices                                      | Fall back to Bearing-only creation and prompt for a replacement destination                              |
| M6.8e     | not-started | Isolate settings across account lifecycle                                | Prevent preference leakage on account switch, sign-out, and deletion                                     |
| M6.9      | not-started | Aggregate Firestore and live device events                               | Device-originated events remain ephemeral and are never mirrored to Firestore                            |
| M6.9a     | not-started | Define discriminated event models                                        | Separate Bearing ownership, device ownership, and merged display behavior                                |
| M6.9b     | not-started | Expand Bearing event fields                                              | Add all-day, timezone, location, recurrence, alarms, availability, and supported URL metadata            |
| M6.9c     | not-started | Query selected calendars by visible range                                | Normalize timed, all-day, recurring, status, capability, and source data                                 |
| M6.9d     | not-started | Merge, sort, color, and deduplicate events                               | Suppress linked system copies while retaining source labels                                              |
| M6.9e     | not-started | Refresh live events at deterministic triggers                            | Refresh on range, resume, settings, successful mutation, and manual action without polling               |
| M6.9f     | not-started | Support device events in Focus Mode                                      | No Firestore document is required for device-event focus sessions                                        |

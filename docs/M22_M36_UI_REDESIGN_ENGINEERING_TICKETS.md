...
| M35.6  | completed | Validate Profile                        | Focused navigation/Profile tests, typecheck, lint, format, and diff pass; full-suite PlanScreen gap recorded below |
...
| 2026-09-08 | M34.1-M34.6         | passed         | Notes now use search/filter controls, active-note Pinned/Recent/All sections without overlap, a route-owned plain-text editor with save/pin/archive/delete confirmation, and editable Task/Goal/Event conversion drafts. Focused Notes/navigation/editor/conversion validation and full mobile Jest pass (63 suites, 435 tests); `npm --prefix mobile run typecheck`; `npm --prefix mobile run lint`; touched-file `npx prettier --check`; and `git diff --check` pass. References: New Note section 15 and Notes List section 21 in `docs/UX_V2_FLOW_MAP_AND_SPEC.md`; Notes panels in `docs/mockups/bearing-ui-overview.png` and `docs/mockups/bearing-ui-screen-flows.png`; commit format in `docs/COMMIT_CONVENTIONS.md`: `type(scope): imperative message`. |
| 2026-09-07 | M35.1-M35.6         | passed-with-gap | Profile keeps the approved grouped hub while typed Personal Information, Security, Connected Services, Preferences, Plan & Billing, Legal, and Subscription routes render focused sections with back navigation. The existing ThemeProvider preference is exposed and persisted from Profile; calendar access, exports, purchases, credits, privacy, legal, account deletion, and sign-out behavior remain reachable. Focused Profile/AppTabs validation passes (35 tests); `npm --prefix mobile run typecheck`, `npm --prefix mobile run lint`, touched-file `npx prettier --check`, and `git diff --check` pass. The full mobile Jest run is blocked by the unrelated existing `PlanScreen.test.tsx` expectation for `Plan block 0`, which also fails when that suite runs alone. References: Profile section 22 and Appearance section 23 in `docs/UX_V2_FLOW_MAP_AND_SPEC.md`; Profile panel 15 in `docs/mockups/bearing-ui-overview.png` and `docs/mockups/bearing-ui-screen-flows.png`; commit format in `docs/COMMIT_CONVENTIONS.md`: `type(scope): imperative message`. |

...
Bottom tab navigation with five screens:
...
- Free, user-selected access to calendars already configured on the device through `expo-calendar`.
- Live device events are not copied into Bearing cloud storage.
- Bearing events may optionally be published to one configured writable system calendar.
- iCalendar (.ics) export and JSON account-data export are available; ICS import is outside v1.
...
- Create and manage unscheduled tasks.
- Schedule a task as a calendar event or start it immediately in Focus Mode.
- Converted tasks remain available through completed-task history.
...
- Honest, retryable coordination between Bearing events and optional system-calendar copies.
